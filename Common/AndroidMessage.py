...
class Android:
    ...
    def get_app_name(self) -> str:
        ...
        with os.popen(f'adb shell pm list packages | findstr {self.app}', 'r') as f:
    ...
    def get_app_Activity(self) -> str:
        ...
        # 调用自身的方法去获取app的包名
        with os.popen(f'adb shell dumpsys package {self.get_app_name} | findstr filter', 'r') as f:
            output = f.read()
            app_activity = output.split()[1].split('/.')[1] # 提取内容主要部分得到主干的activity
            # app_activity = activity_key.split('/.')[1]  # 替换掉非法的输入活动名
        # print(f'{self.app}的启动页Activity为:', app_activity)
        print('app的启动页Activity:', app_activity)
        return app_activity
